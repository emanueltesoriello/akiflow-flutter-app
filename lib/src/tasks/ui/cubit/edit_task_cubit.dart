...
class EditTaskCubit extends Cubit<EditTaskCubitState> {
  ...
  onMinutesChanged(int value) {
    Task task = state.updatedTask;
    int duration =
        timeOfDayToSeconds(TimeOfDay(hour: secondsToTimeOfDay(state.updatedTask.duration ?? 0).hour, minute: value));
    Task updated = task.copyWith(
      duration: duration != 0 ? Nullable(duration) : Nullable(null),
      updatedAt: Nullable(TzUtils.toUtcStringIfNotNull(DateTime.now())),
    );
    emit(state.copyWith(updatedTask: updated));
  }
  ...
  onHoursChanged(int value) {
    Task task = state.updatedTask;
    int duration =
        timeOfDayToSeconds(TimeOfDay(hour: value, minute: secondsToTimeOfDay(state.updatedTask.duration ?? 0).minute));
    Task updated = task.copyWith(
      duration: duration != 0 ? Nullable(duration) : Nullable(null),
      updatedAt: Nullable(TzUtils.toUtcStringIfNotNull(DateTime.now())),
    );
  ...
  /*void toggleDuratdion(BuildContext contexteee, {bool openedFromNLP = false}) {
    ...
    showCupertinoModalBottomSheet(
      ...
      builder: (context) => DurationCupertinoModal(
        ...
        onHoursChanged: (value) {
          Task task = state.updatedTask;
          int duration = timeOfDayToSeconds(
              TimeOfDay(hour: value, minute: secondsToTimeOfDay(state.updatedTask.duration ?? 0).minute));
          Task updated = task.copyWith(
            duration: duration != 0 ? Nullable(duration) : Nullable(null),
            updatedAt: Nullable(TzUtils.toUtcStringIfNotNull(DateTime.now())),
          );
        ...
        onMinutesChanged: (value) {
          Task task = state.updatedTask;
          int duration = timeOfDayToSeconds(
              TimeOfDay(hour: secondsToTimeOfDay(state.updatedTask.duration ?? 0).hour, minute: value));
          Task updated = task.copyWith(
            duration: duration != 0 ? Nullable(duration) : Nullable(null),
            updatedAt: Nullable(TzUtils.toUtcStringIfNotNull(DateTime.now())),
          );
